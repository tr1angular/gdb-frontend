...
def run(request, params):
    ...
    if result_json["ok"]:
        ...
        if "expression" in qs_params.keys():
            variable = debug.getVariableByExpression(qs_params["expression"][0], no_error=True)
        
        if not variable and ("variable" in qs_params.keys()):
            variable = debug.getVariableByExpression(qs_params["variable"][0], no_error=True)
