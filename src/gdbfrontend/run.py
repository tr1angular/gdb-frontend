...
globalvars.terminal_id = terminal_id
credentials = False
...
if value_expected_arg:
    print("Missing value for option:",
          value_expected_arg[0] + (", " + value_expected_arg[1]) if value_expected_arg[1] else "")
    exit(0)

if gdb_executable == "gdb" and not shutil.which("gdb"):
    print(
        "\033[0;32;31m[Error] GDB is not installed. Please install GDB on your system and run GDBFrontend again.\033[0m")
    exit(1)

if tmux_executable == "tmux" and not shutil.which("tmux"):
    print(
        "\033[0;32;31m[Error] Tmux is not installed. Please install Tmux on your system and run GDBFrontend again.\033[0m")
    exit(1)
...
def exiting():
    ...
    quit_tmux_gdb()
    subprocess.Popen([tmux_executable, "kill-session", "-t", terminal_id], stdout=subprocess.PIPE,
...
try:
    ...
    if "--with-python" not in repr(proc.stdout):
        print(
            "[Error] Your GDB (" + gdb_executable + ") doesn't have embedded Python. Please install a GDB version with embedded Python or you can build it yourself.")
    ...
    if not is_random_port:
        ...
        print(("Open this address in web browser: \033[0;32;40mhttp://%s:%d/\033[0m" % (
