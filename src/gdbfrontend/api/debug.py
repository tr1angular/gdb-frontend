...
def threadSafe(no_interrupt=True):
    ...
    def _decorator(callback):
        ...
        def _threadSafe(*args, **kwargs):
            ...
            if not is_mt:
                use_interrupt = not no_interrupt and settings.INTERRUPT_FOR_THREAD_SAFETY and globalvars.debugFlags.get(flags.AtomicDebugFlags.IS_RUNNING)
                ...
                if use_interrupt:
                    ...
                    globalvars.debugFlags.set(flags.AtomicDebugFlags.IS_INTERRUPTED_FOR_THREAD_SAFETY, _exec__mT)
                ...
                while lockCounter.get() > 0:
                    if not is_warned and time.time() - start_time > settings.GDB_MT_WARNING_TIME:
                        if not use_interrupt:
                            is_warned = True
                            globalvars.httpServer.wsSendAll("{\"event\": \"mt_blocking\"}")
                        else:
                            is_warned = True
                            globalvars.httpServer.wsSendAll("{\"event\": \"mt_blocking_with_interrupting\"}")
...
def getState():
    ...
    state["is_enhanced_collabration"] = globalvars.is_enhanced_collabration
    state["collabration"] = {}
    state["collabration"]["state"] = globalvars.collabration_state

    state["breakpoints"] = getBreakpoints()
    state["objfiles"] = getFiles()
    state["sources"] = getSources()
    state["registers"] = getRegisters()
    state["step_time"] = globalvars.step_time
    ...
    if inferior.num in globalvars.inferior_run_times.keys():
        state["inferior"]["run_time"] = globalvars.inferior_run_times[inferior.num]
    else:
    ...
    if th0 is not None:
        ...
        if not th0.is_running():
            ...
            selected_frames = globalvars.debugFlags.get(flags.AtomicDebugFlags.SELECTED_FRAMES)
...
def addBreakpoint(file=None, line=None, address=None):
    ...
    if (file is not None) and (line is not None):
        if is_running:
            globalvars.debugFlags.set(flags.AtomicDebugFlags.IS_INTERRUPTED_FOR_BREAKPOINT_ADD, {
                "file": file,
                "line": line,
            })
...
def delBreakpoint(bp):
    ...
    if is_running:
        globalvars.debugFlags.set(flags.AtomicDebugFlags.IS_INTERRUPTED_FOR_BREAKPOINT_DEL, bp)
        gdb.execute("interrupt")
    else:
        bp.delete()
...
def setBreakpointCondition(bp, condition):
    ...
    if is_running:
        globalvars.debugFlags.set(flags.AtomicDebugFlags.IS_INTERRUPTED_FOR_BREAKPOINT_MOD, {
            "breakpoint": bp,
            "condition": condition
        })
        gdb.execute("interrupt")
...
def setBreakpointEnabled(bp, is_enabled):
    ...
    if is_running:
        globalvars.debugFlags.set(flags.AtomicDebugFlags.IS_INTERRUPTED_FOR_BREAKPOINT_SET, {
            "breakpoint": bp,
...
def run(args=""):
    ...
    globalvars.dont_emit_until_stop_or_exit = True
...
def selectFrame(pc):
    ...
    for _frame in stack:
        if _frame.pc() == pc:
            ...
            frames = globalvars.debugFlags.get(flags.AtomicDebugFlags.SELECTED_FRAMES)
...
def terminate():
    ...
    is_running = globalvars.debugFlags.get(flags.AtomicDebugFlags.IS_RUNNING)

    if is_running:
        try:
            globalvars.debugFlags.set(flags.AtomicDebugFlags.IS_INTERRUPTED_FOR_TERMINATE, True)
            gdb.execute("interrupt")
        except Exception as e:
            print("[Error]", e)
...
def getRegisters():
    ...
    for line in lines:
        ...
        try:
            is_changed = vals[0] in globalvars.changed_registers and vals[1] != globalvars.changed_registers[vals[0]][1]
        except BrokenPipeError:
            globalvars.changed_registers = multiprocessing.Manager().dict()
            is_changed = False
        ...
        globalvars.changed_registers[vals[0]] = vals[1]
